...
\label{ch:input}
Authors have chosen a subset of New York Times articles from year 2000 to 2007 which parts with
abnormal style of writing like obituary content or hard structures to deal like lists are filtered out.
...
\end{enumerate}
Among all dependency paths we are just interested in those which link two named entities.
 So basically we are hopeful that realization of one relation is available somewhere in this 
 path most likely as a verb which relates two mentioned named entities. We collect these kinds of 
 paths and based on some conditions that we have already discussed in ??? DIRT we filter out non-important links.
